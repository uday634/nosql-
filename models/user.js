...
userSchema.methods.addToCart = function (product) {
  ...
  if (cartProductIndex >= 0) {
    ...
    updatedCartItems.push({ productId: product._id, quantity: newQuantity }); // Corrected property name to 'productId'
  }
...
userSchema.methods.removeFromCart = function (productId) {
  ...
  this.cart.items = updatedCartItems; // Update the 'items' property
  return this.save();
};
...
//   getCart() {
//     const db = getDb();
//     const productIds = this.cart.items.map(i => new mongodb.ObjectId(i.productId)); // Convert product IDs to ObjectId
...
//         }
//       }
//       return db.collection('orders').insertOne(order) // Return the promise here
//     })
//       .then(result => {
//         this.cart = { items: [] };
//         return db.collection('users').updateOne(
